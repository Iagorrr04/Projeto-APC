...
#define MAXnumVoo 18               // Máximo de caracteres que um numVoo pode ter
#define MAXlinha  804              // Máximo de caracteres que uma linha nos registros pode ter
#define MAXreservas 850            // Máximo de passageiros que um voo pode ter.
#define MAXvoos 204                // Máximo de voos que podem ser cadastrados
#define MAXnumReserva 20           // Tamaho máximo que a string que contem o número de reserva pode ter.
#define MAXCPF 24                  // Tamanho máximo que a string que contem um cpf pode ter.
#define MAXnome 747                // Tamanho máximo que a string que contem um nome completo pode ter.
#define MAXDataChar 24            // Tamanho máximo que uma string contendo uma data pode ter
#define MAXHoraChar 16            // Tamanho máximo para uma string contendo a hora pode ter
#define MAXClientes 1200           // Quantidae máxima de clientes simultâneos que podemos ter.
#define MAXCharTemp 10              // Tamanho maximo para uma string contendo uma informação temporária, geralmente a ser convertida
...
struct reserva
{
    ...
    char sexo;                          // M = masculino, F = feminio, N = prefiro não me identificar
    Data dataNascimento;
    
    int statusRes; //1 = ativo, 0 para cancelado, 2 = voo que já aconteceu
};
...
struct voo
{
    ...
    int totalReservasVoo;                  // Quantas reservas estão cadastradas, independente de ativas ou não.
    Reserva reservaVoos[MAXreservas];
    
    int status; //0 = cancelado, 1 = ativo, 2 = já aconteceu, 3 = Vagas esgotadas
...
/*=================================================================FUNÇÕES==================================================================*/
// Função para espacar linhas verticalmente.
void espacamentoVertical(int altura)
{
    int s;
    for (s =0; s < altura; s++)
...
// Função para checar a integridade do voos, por enquanto só checa se  o voo já passou.
void checarVoos()
{
    // Verificar se a data de um voo ativo já não passou se sim cancelar.
    int v;
    int r;
    for (v = 0; v < totalVoos; v++)
    {
        if (todosVoos[v].status != 2)
        {
            ...
            {   
                ...
                // Cancelar todas as reservas com aquele índice.
                ...
                // Só exibir uma notificaçãozinha.
                mostrarHub();
...
// Função para limpar tela independente do sistema.
void limparTela()
{
    #if defined(linux) || defined(uinix) || defined(APLE)
        system ("clear");
...
// Função para atualizar os cadastros dos Voos, passar os novos dados alterados nos vetores para o .csv
void atualizarCadastrosReservas()
{

    // Abrir o arquivo e reescrever tudo oque está nele.
    ...
    for (r = 0; r < totalReservas; r++)
    {
        ...
        // Notar que quando um voo é excluído seu índice também é alterado, portanto o índice do voo é "dinâmico"
...
// Função para registrar os clientes dos vetores no arquivo
void registrarDadosClientes()
{
...
// Função para verificar se o um dado CPF já não foi cadastrado, retorna a posição caso ache e -1 caso nao.
int buscarCPFClientes(char CPFABus[MAXCPF])
{   
    ...
    // Se não achar para nem um dos clientes registrados.
    return -1;
}
// Função para alterar a variável que armazena a quantidade total de clientes.
void contarTotalClientes()
{
    totalClientes = 0;
    FILE *cadastrosClientes = fopen(CADASTROS_CLIENTES, "w");
...
/*
    Função para gerar os dados dos clientes, baseado no cadastro das reservas.
...
{   
    ...
    for (r = 0; r < totalReservas; r++)
    {
        ...
        // Caso esse CPF ainda não tenha sido cadastrado.
        ...
        {   
            ...
            // se foi achado agora então a quantidade de reservas é igual a 1
    ...
    // Depois de ter gerado os dados dos clientes e armazenado nos vetores registrálos no arquivo
    registrarDadosClientes();
}

// Função para selecionar e retornar um número de voo existente, SÓ FUNCIONA EM WINDOWS.
int selecionarVooAConWin()
{   
...
// Função para contar a quantidade total de reservas e alterar a variável global.
void contarTotalReservas()
{   
...
// Função para contar a quantidade total de voos e alterar a variável global, e também retornar.
int contarTotalVoos()
{
    FILE *cadastroVoos = fopen(CADASTROS_VOOS, "r");
    char line[MAXlinha];

    // Considerar igual a quantidade de linhas menos o cabeçalho.
    int totalRetornar = 0;
    totalVoos = 0;
...
/*  
    Função para atualizar o arquivo com os cadastros dos voos,
    apaga todas as linhas e escreve o valor dos vetores novamente...
*/
void atualizarCadastrosVoos()
{
    // Abrir o arquivo de maneira que "sobrescreva" o que já está nele.
...
// Função para reveber as informações das reservas nos arquivos.
void receberCadastrosReservas()
{   
...
//Função para receber informção do arquivo com os cadastros dos voos e atribuir aos vetores.
void receberCadastrosVoos()
{       
    ...
    //Após atribuir tudo aos vetores fechar o program!!!
    fclose(cadastrosVoos);
}

//Função para receber a data ,e hora atual da maquina e armazenar nas variáveis globais.
void receberDataEHoraAtual()
{   
    //Variável para receber o total de segundos desde de 1 de janeiro de 1970
    time_t seconds;
    seconds = time(NULL);

    //Variável que vai receber todas as informações da data da máquina.
    struct tm myData;
    myData = *localtime(&seconds); //Função que vai converter os segundos totais retornar na formatação usual.

    //Atribuindo os valores obtidos a variável global Data DataAtual.
    ...
    //Atribuindo os valores obtidos a variável global HoraAtual.
...
//Função para parar a execução do programa pela quantidade de segundos dada.
void atraso(int tempo)
{
...
void barraProgresso()
{
    //Solicitando ao usuário para colocar o terminal em tela cheia, equanto exibe tela de carregamento.
    int progresso = 0;
    int iTemp;
    char pisca;
    int piscaInt = 0;
...
//Função para passar a animação do avião.
void aviao()
{   
...
/*
  Função para iniciar o programa, solicita ao usuário para deixar o terminal em tela cheia, "carrega o programa"
  e passa a animação do avião passando.
*/
void iniciarProg()
{
...
//Função para mostrar o HUD principal superior, vai ser usada durante todo o programa
void mostrarHub()
{
    receberDataEHoraAtual();
...
//Função pra preencher um dado vetor com espaços.
void limparVetorChar(char *vetor, int tamanho)
{
    int c;
    for (c = 0; c < tamanho; c++)
...
// Função para receber a opção que o usuário não windows deseja.
int menuPrincipal()
{
    ...
    do
    {
        mostrarHub();
        printf ("\n");
        printf ("\n                                                               Escolha uma opção abaixo e pressione ENTER");
        printf ("\n                                                                       Navegue com: ^W^ e vSv");
        printf ("\n");
        printf ("\n                                                                          n1. Cadastrar voo");
        printf ("\n");
...
//Função para receber a opção do menu principal de um usuário windows.
int menuPrincipalWin()
{   
    char posL[10];
    char posR[10];
...
//Função que recebe o número de um voo como parâmetro e verifica se o mesmo é válido, 1 para válido, 0 para inválido, -1 para caso seja devido a já existir um voo cadastrado com o número dado
/*
    Função que dado um numVoo verifica se o mesmo válido e retorna 1 caso seja válido, e um dos seguintes valores negativos caso não
    -1, tamanho nõa apropriado
    -2, posição do '-' incorreta
    -3, as três primeiras letras não formama palavra JEB
    -4, os 4 últimos dígitos tão estão de acordo
    -5, o numVoo a ser  cadastrado já existe
*/
int validarNumVoo(char numVooAVal[MAXnumVoo])
{   
    // De acordo com a formatação da string.
    ...
    // Verificando se já não existe um voo com o mesmo numVoo.
    int v;
    for (v = 0; v < totalVoos; v++)
    {   
    ...
    //Considerando que não tenha sido barrado em nem uma das condições acima.
    return 1;
}

//Função para receber o número de um Voo, retorna o mesmo caso ele seja apropiado.
void cadastrarNumVoo(char *numVooACad)
{   
    //Parte visual.
    mostrarHub();
    ...
    do
    {
        ...
        if (verify == -5)
        {
            printf ("\n                                                    ! Já existe um voo cadastrado com esse número, tente novamente !");
        }
        else if (verify <= 0)
        {
            printf ("\n                                                               ! Número de voo inválido tente novamente !");;
        }
    }while(verify != 1);
    
}

// Funcão para receber, validar e retornar o número máximo de passageiros de um vetor
int cadastrarMaxPass()
{
    //Parte visual.
    mostrarHub();
    ...
    do
    {
        printf ("\n                                                              Insira abaixo o número máximo de passageiros:");
        printf ("\n                                                                               >> ");
        fflush(stdin);
        ...
        {
            printf ("\n                                                      ! Quantidae máxima de passageiros inváida, tente novamente !");
        }
        else
        {
            break;
...
//Função para verificar se um ano é bisexto.1 para sim, 0 para não
int validarAnoBisexto(int anoAVal)
{
    if (anoAVal % 4 == 0)
    {
...
/*
    Função para verificar se uma data é válida, recebe uma struct data, e altera os valores da mesma, retorna nos seguintes valores.
    2, caso seja no mesmo dia.
    1, caso seja uma data válida e futura.
    -1, para caso um dos valores seja negativo.
    -2, para caso seja numa data incorreta, em questão.
    -3, para caso seja uma data passada.               ********
    -4, para caso o mes seja maior que 12.
    -5, caso a data não esteja no tamanho certo.
    -6, posição das barras.
    -7, número correspondente ao dia ser inválido.
    -8, número correspondente ao mes ser inválido.
    -9, número correspondente ao ano ser inválido.
*/
int validarData(Data *DataAVal)
{   
    /*----------------------------------PARTE CHAR--------------------------*/
    ...
    // Um dos dígitos não serem números.
    ...
    //Retrona 1 caso seja uma data válida e que ainda não aconteceu.
    return 1;
}

//Função para cadastrar a data de um Voo e armazenar no último voo do vetor.
Data cadastrarDataVoo()
{   
    //Parte visual.
    mostrarHub();
    ...
    do
    {   
        //Receber a data inteira como uma string só e depois dividir em dia, mes e ano.
        ...
        if (verify == 2)
        {
            printf("\n                                                   ! Não permitimos cadadastrar um voo no mesmo dia, tente novamente !");
        }
    }while(verify != 1);

    // Considerando que foi feita uma data válida,  retornála
    return dataACad;
}

/*
    Função para verificar se a hora deum voo é válida, retorna os seguintes valores.
    1, válida e anterior a da ta atual da máquina.
    2, válida porém posterior a data atual da máquina.
    0, segundo a formatação padrão de hora.
    -1, inválido devido ao tamano da string fornecida.
    -2, posição dos ":"
    -3, caracter inválido nas horas;
    -4, caracter inválido nos minutos;
*/
int validarHoraVoo(Hora *HoraAVal)
{   
    /*-----------------------------------------------------------CHAR------------------------------------------------------------------*/
    ...
    //Primeiro verificando se o formato é válido
    ...
    // Válida verificar se é anterior a data atual.
    ...
    //Considerando que não tenha sido barrada nos testes acima é uma data válida.
    return 1;
}

//Função para cadastrar a hora de um novo voo.
Hora cadastrarHoraVoo()
{
    ...
    do
    {
        ...
        if (verify <= 0)
        {
            printf ("\n                                                               ! Hora de partida inválida, tente novamente !");
        }
...
//Função para cadastrar o status do voo, ativo = 1, cancelado = 0.
int cadastrarStatusVoo()
{
    //Parte visual.
    mostrarHub();
    ...
    // Considerando que tenha chegado num valor válido retorná-lo
    if (status == '1'){return 1;}
    else {return 2;}
}

//Função para pegar os dados do vetor do último voo e escrever no cadastro de voos.
void registrarVoo(Voo vooACad)
{   
    todosVoos[totalVoos] = vooACad;

    //Abrir o cadastroVoos no modo append, já direciona na última linha.
...
//Função para cadastrar um Voo, e todas as suas variáveis.
void cadastrarVoo()
{   
    //Parte visual.
    mostrarHub();
    ...
    vooACad.atualPass = 0; //Considerando que o voo está sendo cadastra agora, nao tem nem uma vaga ocupada
    vooACad.totalReservasVoo = 0; //Considerando que o voo está sendo cadastrado agora nao tem nem uma reserva feita.
...
/*
    Função que dada um número de Voo verifica se o mesmo e válido,
    -1, para formatação do numVoo inválida.
    -2, para caso o numVoo não esteja cadastrado.
    -4, caso ocorra um erro
    int V, com a posição do voo nos vetores caso seja válido
*/
int acharNumVoo(char numVooAA[MAXnumVoo])
{
    ...
    //A formatão não é válida, e também não é um num que já existe.
    if ((statusNumVoo != 1) && (statusNumVoo != -5))
    {
        return -1;
    }

    //A formatação é válida no entanto o numVoo não está registrado.
    if (statusNumVoo == 1)
    {
        return -2;
    }

    //O numVoo já existe, portanto a formatação é válida, buscar nos vetores qual é a sua posição e retorná-la
    if (statusNumVoo == -5)
    {
        int v;
        for (v = 0; v < totalVoos; v++)
    ...
    //Caso nem uma das condições acima seja atendida retornar -4 de erro.
    return -4;
}

//Função para exibir as informações de um dado voo, limpa a tela insire o hub e mostra as informações.
void exibirDadosVoo(int vooX)
{
    mostrarHub();
    printf ("\n                                                                       >>>> Consultando Voo <<<<");
    printf ("\n");
    printf ("\n                                                                >> Aqui vão as informações do seu voo <<");
    printf ("\n");
    printf ("\n                                                                             Número do voo");
...
// Função para exibir as reservas ativas de um dado voo, localizado pela posição no vetor, retonar a quantidade de reservas exibidas
int exibirReservasAtivas(int iVoo)
{
    printf ("\n\n                                                                          >> Reservas Ativas <<");
...
//Função para cosultar as informações do voo, essa por sua vez vai chamar diversas funções auxliares para mante a integridade do programa.
void consultarVoo()
{
    mostrarHub();
    printf ("\n                                                                       >>>> Consultando Voo <<<<\n");
    ...
    // Selecionar/digitar número do voo de acordo com o OS.
    #if defined(_WIN32) || defined(_WIN64)
        indiceVooACon = selecionarVooAConWin();
    #else
        indiceVooACon = receberNumVooACon();
    #endif
    // Com  a posição do voo escolhido basta exibir os dados do voo.
    exibirDadosVoo(indiceVooACon);

    // Na mesma lógica exibir as reservas ativas.
    int spc;
    ...
    {
        printf ("\n                                                                 ! ESSE VOO NÃO POSSÚI RESERVAS ATIVAS !");
    }
    //Printar isso no final da dela caso dê espaço
...
/*
    Função para receber, validar, e "retornar" o número de um voo, e retornar de fato o indice desse voo nos vetores.
    validar inclui verificar se o mesmo ainda tem ocupação disponível...
*/
int cadastrarNumVooRes(char *numVooRes)
{
    ...
    do
    {
        ...
        printf ("\n                                                       Digite o número do voo em que deseja realizar uma reserva,");
        printf ("\n                                                                          no formato JEB-XXXX");
        printf ("\n                                                                           >> ");
        fflush(stdin);
        ...
        if (verify == -1)// Formatação inválida.
        {
            printf ("\n                                                          ! Número de inserido não é válido, tente novamente !");
        }
        else if (verify == -2)// NumVoo não está cadastrado.
        {
            printf ("\n                                              ! O número de voo inserido não está cadastrado no sistema, tente novamente !");
        }
        else if (verify == -4)// Erro inesperado
        {
            printf ("\n                                                                        ! ERRO, tente novamente !");
        }
        else if (verify >= 0)// Se for um número válido (verify > 0) verificar se o voo não está cheio ou cancelado
        {   
            ...
            {   
                printf ("\n                                                            ! O voo escolhido já está cheio, tente novamente !");
                verify = -4;
            }

            if (todosVoos[verify].status == 0)
            {
                printf ("\n                                                            ! O voo escolhido não está ativo, tente novamente !");
                verify = -4;
            }
        }
    ...
    // Considerando que o numVoo para a reserva seja válido retornálo.
    return verify;
}

/*
    Função para verificar se um número de reserva já existe independente do voo, considerando que esteja em uma formatção válida, retorna:
    -1, caso o número de reserva não encontrado nos registros.
    int pos, maior ou igual a zero com a posição da linha em que aquele número de reserva se encontra.
*/
int buscarNumRes(char numResAbus[MAXnumReserva])
{   
    // Armazenar as informações de cada linha na reserva temp e verificar se a o numRes daquela linha é igual ao fornecido.
    Reserva reservaTemp;
    FILE *cadastrosReservas = fopen(CADASTROS_RESERVAS, "r");
    char lineTemp[MAXlinha];
    ...
    // Se chegar até aqui é por que não foi encontrado nos registros.
    return -1;
    fclose(cadastrosReservas);
}
/*
    Função para validar um número de reserva, vai recebeloe e retornar:
    1, caso seja um número de reserva válido;
    -1, caso não esteja do tamanho adequado;
    -2, caso os 2 caracteres do início estejam incorreto;
    -3, caso "-" não esteja na posição correta;
    -4, caso os últimos 6 dígitos não sejam números.
    -5, caso já exista um número de reserva igual.
*/
int validarNumRes(char numResAVal[MAXnumReserva])
{
    // Tamanho.
    ...
    // Verificando se já não existe esse mesmo número de reserva
    if (buscarNumRes(numResAVal) != -1){return -5;}

    // Considerando que não foi barrado, é um número de reserva válido.
    return 1;

}

// Função para receber o número de uma reserva, e "retorna-lo", caso seja válido.
void cadastrarNumRes(char *numResACad)
{   
    mostrarHub();
    printf ("                                                                      >>>> Cadastro de Reserva <<<<\n");
    ...
    do
    {
        printf ("\n                                                            Digite o número da reserva que está realizando,");
        ...
        if (verify == -5)
        {
            printf ("\n                                                    ! Já existe um número de reserva igual a esse, tente novamente !");
        }
        else if (verify < 0)
        {
            printf ("\n                                                            ! Número de reserva inválida, tente novamente !");
        }
...
/*
    Função que "recebe" um CPF e retorna valores negativos caso inválido e 1 caso válido
    -1, caso o seja tamanho inválido;
    -2, caso um dos três primeiros dígitos sejam inválidos;
    -3, caso a posição do primeiro . seja inválida;
    -4, caso um dos três dígitos do meio sejam inválidos;
    -5, caso a posição do segundo ponto seja inválida;
    -6, caso um dos três últimos dígitos seja inválido;
    -7, caso a posição do '-' seja inválido;
    -8, caso um dos dois últimos dígitos seja inválido;
    -9, caso a validação do CPF pelo primeiro dígito seja inválido;
    -10, caso a validação do CPF pelo segundo dígito seja inválida;
    -11, caso seja um CPF com todos os números iguais;
    1, caso o CPF seja válido.
*/
int validarCPF(char* CPFAVal)
{
    // Tamanho.
    ...
    // Considerando que a formatação seja válida, verificar se obedece as regras do CPF

    // Gerando string somente com os números, para facilitar o processo
    char CPFNum[12];
    int i;
    int c = 0;
    for (i = 0; i < 14; i++)
    ...
    // Verificando se não é um CPF com todos os números iguais.
    int totalIguais;
    for (i = 0; i < 10; i++)
    {   
        totalIguais = 0;
    ...
    // Considerando que não foi barrado em nenhuma das verificações acima o CPF é válido.
    return 1;
}

/* 
    Função para verificar se já existe alguma reserva num dado voo, com um dado CPF, retorna:
    1, se o CPF não foi encontrado nas reservas do Voo;
    -1, caso CPF seja encontrado em umas das reservas, e reserva está ativa;
    -2, caso o CPF seja encontrado em umas das reservas porém a reserva não esteja ativa.
*/
int buscarCPFEmVoo(char CPFAbus[MAXCPF], int iVoo)
{
    Reserva reservaTemp;
    ...
    // Considerando que não foi encontrado um CPF igual retorna verdadeiro.
    return 1;
}

/*  
    Função para receber um CPF, até que seja válido retorna os seguintes valores.
    -1, caso seja um CPF que já existe, em outro voo.
...
int cadastrarCPF(char *CPFAcad, int inVoo)
{
    ...
    do
    {
        ...
        if(verify < 0)
        {
            printf ("\n\n                                                              ! Número de CPF inválido tente novamente !");
        }
        else // Se for um CPF válido verificar se já não foi usado em outra reserva, ATIVA.
        {
            if (buscarCPFEmVoo(CPFAcad, inVoo) == -1)
            {
                printf ("\n                                                   ! Já existe uma reserva ativa cadastrada neste voo com este CPF !");
                printf ("\n                                                                           tente novamente");
                verify = -1;
            }
    ...
    // Um cpf novo e não registrado no voo foi inserido.
    return 1;
}   
/*
    Função que dado o endereço de um nome retorna um valor inteiro de acordo com a validação do nome.
    1, caso seja um nome válido.
    -1, cao seja um nome inválido
*/
int validarNome(char *nomeAVal)
{   
    // Verificando se não existe nem um caracter inapropriado, considerando que seja um nome "comum".
    int i;
    ...
    {   
        ...
        {   
            printf ("\nCARACTER INVÁLIDO: %c", nomeAVal[i]);
            return -1;
        }
    }


    // Considerando que não foi barrado em nem uma das verificações acima, o nome é válido.
    return 1;
}

// Função para receber e adicionar o nome a um dado endereço do char, solicita nomes até que seja inserido um nome válido.
void cadastrarNome(char *nomeACad)
{
    mostrarHub();
    printf ("                                                                      >>>> Cadastro de Reserva <<<<\n");
    ...
    do
    {
        ...
        if (verify != 1)
        {
            printf ("\n                                                           ! O nome inserido não é válido, tente novamente !");
        }
    }while(verify != 1);
}


// Função que retorna um caractere conrrespondente ao sexo do usuário, solicita um caractere válido até conseguir.
char cadastrarSexo()
{
    mostrarHub();
    printf ("                                                                      >>>> Cadastro de Reserva <<<<\n");
    ...
    do
    {
        ...
        printf ("\n                                                                             n1. Masculino");
        printf ("\n                                                                             n2. Feminino");
        printf ("\n                                                                    n3. Prefiro não me indentificar");
        printf ("\n                                                                          >> Inisira aqui: n");
        fflush (stdin);
        sexoACad = getc(stdin);
...
// Função para cadastrar e retornar uma data de nascimento válida
Data cadastrarDataNascimento()
{
    mostrarHub();
    printf ("                                                                      >>>> Cadastro de Reserva <<<<\n");
    ...
    do
    {
        ...
        if (verify != -3) //-r é para datas passadas
        {   
            printf ("\n\n                                                          ! A data inserida não é válida, tente novamente !");
        }
        else // Considerando que seja uma data que já passou verifiacar se tem mais de 84 anos.
        {
            if ((DataAtual.ano - dataNasACad.ano) > 84)
            {   
                printf ("\n\n                                                        ! Não são permitidos passageiros acima de 84 anos !");
                printf ("\n                                                                            tente novamente");
            ...
            {
                printf ("\n\n                                                    ! Não permitimos criançãs abaixo de 10 anos, tente novamente !");
                verify = -9;
            }
        }
    ...
    // Retornar a data de nascimento já validada.
    return dataNasACad;
}

// Função para cadastrar o status de uma reserva, retorna o inteiro de acordo com o mesmo.
int cadastrarStatusReserva()
{
    ...
    do
    {
        ...
        {
            printf ("\n\n                                                            ! O status inserido é inválido, tente novamente");
        }
    }while ((statusACad != '1') && (statusACad != '2') );

    // Sempre retornar o valor ao finaldas funções de cadastro.
    if (statusACad == '1'){return 1;}
    else {return 0;}
}

// Função que dada uma reserva adiciona a mesma no arquivo de reservas na última posição, e adiciona a mesma ao vetor do voo a qual pertence.
void registrarReserva(Reserva reservaAReg)
{
    ...
    // Agora cuidar da parte de registrar no arquivo, abrir o arquivo apontando na próxima linha e escrever as informações da reserva.
    FILE *cadastroReservas = fopen(CADASTROS_RESERVAS, "a");

    // NÃO ESQUCER DO '\n' PARA APONTAR PARA A PRÓXIMA LINHA !!!!
...
// Função para escolher um voo do jeito  "cool"
int selecionarNumVooRes()
{
    char indicadoresE[MAXvoos];
    char indicadoresD[MAXvoos];
    ...
    while (1)
    {
        ...
        {   
            ...
            else if (todosVoos[pos].status == 2)
            {
                ...
                printf ("                                                                      >>>> Cadastro de Reserva <<<<\n");
                printf ("\n\n                                                                          >> Selecione o Voo <<\n");
                printf ("\n                                                            ! Voo escolhido já aconteceu, tente novamente !");
                espacamentoVertical(25);
                printf("\n                                                                   >> PRESSIONE ENTER PARA CONTINUAR <<");
                fflush(stdin);
                getc(stdin);
...
/*
    ...
    vai chamar as funções para cadastrar cada valor de uma reserva.
...
void cadastrarReserva()
{
    ...
    // Recebendo numVoo da reserva e o indice do mesmo numa paulada só !
    #if defined(_WIN32) || defined(_WIN64)
    ...
    else // É um cliente novo, registrar todos os dados.
    {
    ...
    do
    {
        ...
        printf ("                                                                      >>>> Cadastro de Reserva <<<<\n");
        printf ("\n                                                                          >> Sua Reserva <<");
        printf ("\n                                                                      Número Reserva:%s",reservaACad.numRes);
        printf ("\n                                                                      Número Voo:%s", reservaACad.numVooRes);
    ...
    // De acordo com a confirmação ou não.
    if (confirma == 1)
    {    
...
// Função para recber confirmação de um voo, 1 pra sim 0 para não.
int confirmarCadastroVoo(Voo VooACon)
{
    int confirma;
...
// Função que simplesmente exibe os dados de uma dada reserva.
void exibirDadosReserva(int indiceResAEx)
{
    // Número do voo da reserva.
    printf ("\n                                                                           >> Número do voo <<");
    printf ("\n                                                                                %s", todasReservas[indiceResAEx].numVooRes);
    printf ("\n");

    // Número da reserva em questão.
    printf ("\n                                                                         >> Número da reserva <<");
...
// Função para receber a reserva a ser consultada, e retornar o índice.
int receberNumReservaACon()
{
    // Estética.
    mostrarHub();
    ...
    // Primeiro achar qual o número da reserva.
    char numResACon[MAXnumReserva];
    int indiceReserva;
    do
    {
        printf ("\n                                                           Qual o número da reserva que deseja consultar ?");
        printf ("\n                                                                >> Insira aqui: ");
        fflush(stdin);
        ...
        if (indiceReserva != -5) // Exibir mensagem de acordo com a invalidez.
        {
            if (indiceReserva == 1)
            {
                printf ("\n                                                       ! Não achamos nem uma reserva cadastrada com esse número !");
                printf ("\n                                                                            tente novamente");
                indiceReserva = -1;
            }
            else
            {
                printf ("\n                                                             ! Número de reserva inválido, tente novamente !");
            }
        }
        else // Caso seja um número de reserva válido descobrir qual é a posição do mesmo.
        {
    ...
    // Retornar o índice conseguido.
    return indiceReserva;
}

// Função pra selecionar reserva a ser consultada.
int selecionarReservaACon()
{
    int i;
...
/*
    Função para consultar uma reserva, solicita o número até ser válido, e então exibe os dados.
*/
void consultarReserva()
{   
    // Estética.
    ...
    //Con o indice do número da reserva "em mãos" basta exibir os dados.
...
// Função para exbir dados gerais dos clientes.
...
// Função para exibir TODAS as reservas de um cliente, específicado pelo CPF.
void exibirReservasCliente(char CPFCliente[MAXCPF])
{   
    ...
    for (r = 0; r < totalReservas; r++)
    {
        ...
        if (strcmp(CPFCliente, todasReservas[r].CPF) == 0)
        {
            ...
            if (spc == 0)
            {
                printf ("\n                                                | Número da Reserva | Número do Voo | Data do voo | Status da Reserva |");
            }
            spc++;  
            printf ("\n                                                |     %s     |   %s    | %s  | %s| ", 
                                        todasReservas[r].numRes,
...
void consultarCliente()
{
    ...
    // Receber CPF até que seja inserido um válido.
    ...
    do
    {
        ...
        if (verify != 1)
        {
            printf ("\n                                                           ! O CPF inserido não é válido tente novamente !");

        }
        else // Se for válido verificar se o CPF pertence a um cliente.
        {
            verify = buscarCPFClientes(CPFClienteACons);
            if (verify == -1)
            {
                printf ("\n                                                     ! O CPF fornecido não corresponde a nem um de nossos clientes !");
                printf ("\n                                                                            tente novamente");
            }
        }
    }while(verify < 0);

    // Exibir dados gerais do cliente, tendo em mente que a "verify" agora contem a posição do cliente nos vetores.
...
// Função para receber o número de voo a cancelar, não se encerra até receber um número de voo válido.
void receberNumVooACancelar(char *numVooACan)
{
    // Estética.
    mostrarHub();
    ...
    do
    {
        printf ("\n                                                                 >> Digite o número de voo a cancelar <<");
        printf ("\n                                                                            >> ");
        fflush(stdin);
        ...
        {
            printf ("\n                                                    ! O número do voo inserido não foi encontrado no nosso sistema !");
            printf ("\n                                                                             tente novamente");
        }
        else if (verify == -4)
        {
        ...
        else // Caso válido verificar se o voo já não está cancelado
        {
            if (todosVoos[verify].status == 0)
            {
                printf ("\n                                                   ! O número do voo inserido, corresponde a um voo já cancelado !");
                printf ("\n                                                                             tente novamente");
                verify = -1;
            }
            else if (todosVoos[verify].status == 2)
            {
                printf ("\n\n                                                    ! O número do voo corresponde a um voo que já aconteceu !");
                printf ("\n                                                                            tente novamente");
                verify = -1;
            }
...
//Função para confirmar o cancelamento de um voo, 1 pra sim, 0 para não.
int confirmarCancelamentoVoo(Voo VooACan)
{
    int confirma;
    char confirmaCharTemp[10];
...
// Função para selecionar e retornar um número de ativo, SÓ FUNCIONA EM WINDOWS, devido a conio.h
int selecionarVooACanWin()
{   
    char indicadoresE[MAXvoos];
    char indicadoresD[MAXvoos];
    ...
    while (1)
    {
        ...
        if (posChar == 13)
        {
            ...
            else if (todosVoos[pos].status == 2)
            {
                mostrarHub();
                printf ("\n                                                                        >>>> Cancelando Voo <<<<\n");
                printf ("\n                                                               ! O Voo escolhido já passou, ecolha outro !");
            ...
            else if (todosVoos[pos].status == 0)
            {
                mostrarHub();
                printf ("\n                                                                        >>>> Cancelando Voo <<<<\n");
                printf ("\n                                                           ! O Voo escolhido já foi cancelado, ecolha outro !");
                espacamentoVertical(30);
                printf ("\n                                                         >> PRESSIONE ENTER PARA PROSSEGUIR COM O CADASTRO <<");
                fflush(stdin);
                getc(stdin);
...
// Função para cancelar um voo, no geral chama as demais funções.
void cancelarVoo()
{
    // Estética.
    mostrarHub();
    ...
    // Receber número do voo a cancelar.
    int indiceVooACan;
    #if defined(_WIN32) || defined(_WIN64)
    ...
    // Confirmar ou abortar operação de acordo.
    if (confirmarCancelamentoVoo(todosVoos[indiceVooACan]) == 1)
    {   
        // Cacelar o voo baseado no número fornecido.(só alterar o status.)
        todosVoos[indiceVooACan].status = 0;

        // Já que o voo foi cancelado a quantidade de reservas agora também é zero.
...
// Função para receber um número de voo a ser cancelado e retornar sua posição.
int receberNumVooACon()
{
    int verify;
    ...
    {   
        ...
        //Verificando se a entrada é válida e imprimindo mensagem de acordo.
        mostrarHub();
        printf ("\n                                                                       >>>> Consultando Voo <<<<\n");
        ...
        {
            printf ("\n                                                    ! O número de voo inserido não corresponde a nenhum cadastrado !");
            printf ("\n                                                                          ! Tente novamente !");
        }
        else if (verify == -4)
        {
...
// Função para receber um número de reserva a cancelar.
void receberNumResACan(char *numResACan)
{
    int verify;
    mostrarHub();
    ...
    do
    {
        ...
        if (verify != -5)
        {
            if (verify == 1)
            {
                printf ("\n                                                         ! Não foi encontrada nem uma reserva com esse número !");
                printf ("\n                                                                             tente novamente");
            }
            else
            {
                printf ("\n                                                             ! O número de reserva inserido não é válido !");
                printf ("\n                                                                             tente novamente");
            }
        }
        else    // Caso seja válido verificar se a reserva já não está cancelada
        {
            int posReserva = buscarNumRes(numResACan);
            if (todasReservas[posReserva].statusRes == 0)
            {
                printf ("\n\n                                                      ! A Reserva correspondente a esse número já está cancelada !");
                printf ("\n                                                                            tente novamente");
                verify = -1;
            }
            else if (todasReservas[posReserva].statusRes == 2)
            {
                printf ("\n\n                                                  ! O número de reserva inserida corresponde a um voo que já passou !");
                printf ("\n                                                                            tente novamente");
                verify = -1;
...
//Função para cancelar o cancelamento de uma reserva, 1 para sim, 0 para não
int confirmarCancelamentoReserva(Reserva ResACan)
{

    // Receber opção do usuário até ser válido.
    int confirma;
...
// Função para cancelar uma reserva, baseado no número da reserva, chama as outras reservas.
void cancelarReserva()
{   
    // Estética.
    mostrarHub();
    ...
    // Receber o número da reserva a ser cancelado.
    char numResACan[MAXnumReserva];
    receberNumResACan(numResACan);

    // Cancelar ou abordar, de acorodo com a confirmação.
    ...
    if (confirmarCancelamentoReserva(todasReservas[buscarNumRes(numResACan)]) == 1)
    {
        ...
        //Atualizar todos os dados necessários
        totalReservasAtivas--;
        atualizarCadastrosVoos();
        atualizarCadastrosReservas();
        gerarDadosClientes();
...
// Função que recebe um número de voo a ser excluído até que seja válido e altera a variável fornecida.
void receberNumVooAExc(char *numVooAExc)
{
    mostrarHub();
    printf ("\n                                                                         >> Excluir Voo <<\n");
    ...
    do 
    {
        printf ("\n                                                                 Digite o número do voo que deseja excluir");
        printf ("\n                                                                            no formato JEB-1234");
        printf ("\n                                                                             >> ");
        fflush(stdin);
        ...
        if (verify == -1)
        {
            printf ("\n                                                                 ! Número de voo inserido não é válido !");
            printf ("\n                                                                              tente novamente");

        }
        else if (verify == -2)
        {
            printf ("\n\n                                                  ! O número de voo inserido não corresponde a nemhum voo cadastrado !");
            printf ("\n                                                                              tente novamente");
        }
    }while(verify < 0);
}

// Função que dado o índice de um voo nos vetores o remove.
void excluirDadosVoo(int indiceVooAExc)
{   
    if (todosVoos[indiceVooAExc].status == 1){totalVoosAtivos--;}

    // Só tomar cuidaddo caso o voo a ser removido seja o último.
...
// Função que dado um índice de um voo escreve todas as reservas com execção das pertencentes a esse voo.
...
{   
    ...
    // Todas as reservas com execção das pertencentes ao voo fornecido.
    int r;
    for (r = 0; r < totalReservas; r++)
    {   
...
// Função para confirmar a exclusão de um voo, 1 para sim 0 para não.
int confirmarExclusaoVoo(Voo VooAEx)
{
    // Estética.
    mostrarHub();
...
// Função para selecionar o voo a ser excluído.
int selecionarVooAExcWin()
{   
    char indicadoresE[MAXvoos];
    char indicadoresD[MAXvoos];
...
/*
    Função para excluir um voo, vai chamar diversas funções e então realizar as seguintes operaçoes.
    - Receber um número de voo válido a ser excluído.
...
void excluirVoo()
{
    ...
    // Recebendo número do voo que vai ser excluído.
    ...
    // Confirmando exclusão do voo.
    if (confirmarExclusaoVoo(todosVoos[acharNumVoo(numVooAEx)]) == 1)
    {        
            // Remover voo dos vetores.
            excluirDadosReservas(numVooAEx); // excluir os dados das reservas primeiro, pois se não não encontra o número do voo.
